...
Після того як створили ексемпляр, створюємо саму базу. У вкладці Databases натискаємо кнопку CREATE DATABASE. Називаємо базу cloud-run-backend-example.
<p align="center">
    <img alt='create db' src="./images/createDB.png" width="50%" >
</p>

Збережемо собі дані для використання їх як змінні оточення в нвшому beckend сервісі:
- db user в нашому випадку створився за умовчанням - root (as env DB_USERNAME)
- збережемо пароль для бази, який ми раніше згенерували при створенні екзкмпляра (as env DB_PASSWORD)
- також збережемо публічний IP adress, він вказаний в описі екземпляра (as env DB_HOST)
...
Ми вже на основній частині створення наного проекту. Перед цим ми готували основу, створюючи базу даних , сховище для файлів та підготували готові образи для 
...
рекомендовано використовувати інтеграцію [Secret Manager API](https://cloud.google.com/secret-manager/docs).
<p align="center">
...
Після створення сервісу нам доступний інтерфейс з ревізіями, де ми можемо керувати трафіком. Також нам створюється унікальне посилання на наш сервіс.
...
- у вкладці CONTAINER для поля Container port вказуємо порт під номером 3000. Саме під цим портом у акщтеутв контейнері у нас стартує NEXT.js server. З таким налаштуванням
...
Допрацювати
- створення бази даних та підключення через connections
- поміняти картинку для Dockerfile
- висновок

https://cloud.google.com/sql/docs/mysql/connect-instance-cloud-run#node.js
